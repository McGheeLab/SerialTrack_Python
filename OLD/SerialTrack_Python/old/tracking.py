"""
SerialTrack Python — Main tracking engine (v2)
================================================
    serialtrack/tracking.py

Fixes from v1:
  - f_o_s now actually updates between ADMM iterations (was a no-op)
  - Strain computed in BOTH reference and deformed configurations
  - Double-frame mode (TrackingMode.DOUBLE_FRAME) implemented
  - Trajectory building integrated into TrackingSession
  - Imports use canonical config.GlobalSolver (no duplicate)
"""
...
from .fields import (
    ...
    compute_strain_mls, compute_gridded_strain, scatter_to_grid_multi,
)
...
class FrameResult:
    ...
    # Post-processing (reference config — was missing in v1)
...
class _ADMMFrameTracker:
    ...
    def run(
        ...
        # FIX: working copy of f_o_s that actually updates per iteration
        ...
        for iter_num in range(cfg.max_iter):
            ...
            # ── FIX: actually update f_o_s for next iteration ──
            if len(temp_disp) > 0:
                working_f_o_s = update_f_o_s(temp_disp, f_o_s_min=60.0)
    ...
    def _local_step(self, coords_a, coords_b_curr, not_missing_a, not_missing_b,
        ...
        n_max_local = self.cfg.n_neighbors_max

        matches_raw = np.empty((0, 2), dtype=np.int64)

        while len(matches_raw) == 0 and n_max_local < len(not_missing_a):
            if n_neighbors > 2:
            ...
            if len(matches_raw) == 0:
                n_max_local += 5
                n_neighbors = min(n_neighbors, n_max_local)
            else:
...
class SerialTracker:
    ...
    def _run_tracking(
        ...
        for fi in range(1, n_frames):
            ...
            if cfg.mode == TrackingMode.CUMULATIVE:
                ...
                # Each pair is independent: always frame i-1 → frame i
            ...
            # Strain in both configs (FIX: was only deformed config)
    ...
    def _compute_strain(self, result: FrameResult, coords_a: np.ndarray,
        ...
        MATLAB does:
          funCompDefGrad3(disp, parCoordB, ...)          → deformed config
          funCompDefGrad3(disp, parCoordB - disp, ...)   → reference config
        """
